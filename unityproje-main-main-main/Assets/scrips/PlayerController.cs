   ...
   void FizikVeHareketKontrolu()
    {
        ...
        // --- YENİ EKLENEN: Kameraya Göre Hareket Mantığı ---
        Transform kamera = Camera.main.transform;
        Vector3 kameraIleri = kamera.forward;
        Vector3 kameraSag = kamera.right;

        // Y eksenini (yukarı/aşağı) sıfırlıyoruz ki karakter eğilmesin
        kameraIleri.y = 0f;
        kameraSag.y = 0f;
        kameraIleri.Normalize();
        kameraSag.Normalize();

        // Hareket vektörünü kameranın bakış açısına göre oluşturuyoruz
        ...
        if (isMoving)
        {
            // Karakteri de kameranın yönüne uygun olarak döndürüyoruz
